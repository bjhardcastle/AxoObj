...
for tidx = 1:length(obj.TrialStartSample)
    ...
    ch1(tidx) = mean( wstim( obj.TrialStartSample(tidx) : obj.TrialEndSample(tidx) ,1) );
    ...
    ch7(tidx) = mean(wstim( obj.TrialStartSample(tidx) : obj.TrialEndSample(tidx) ,7) );
         ...
         % Get ch9,10 mean voltage within trials 
    ch10(tidx) = mean(wstim( obj.TrialStartSample(tidx) : obj.TrialEndSample(tidx) ,10) );
    ch11(tidx) = mean(wstim( obj.TrialStartSample(tidx) : obj.TrialEndSample(tidx) ,11) );
...
if isprop(obj,'SwitchPatSeqAbfChans') && ~isempty( obj.SwitchPatSeqAbfChans ) && obj.SwitchPatSeqAbfChans == 1
    ...
    obj.TrialSeqNum =  (ch2*20/5);
else
    obj.TrialPatNum = round(ch1*20/5);
    obj.TrialSeqNum =  (ch7);
end
...
ch10round = round(ch10,1);
polAng = round(ch10round.*360/5);
polAng = round(polAng/7.5)*7.5;
unique(polAng)
polSign = round(ch11) -2;
obj.TrialPolAng = polAng.*polSign;
...
%temporary:
% obj.polAng = wrapTo180(obj.polAng -360 )
obj.polAng = sort(unique(obj.TrialPolAng));
% assert(isequal(obj.polAng, sort(unique())) )
% disp('using temporary polang calc: remove after june 11')
