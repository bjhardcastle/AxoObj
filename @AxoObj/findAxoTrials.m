function trialIdx = findAxoTrials(obj,fields)
...
all_trials = (1:length(obj.TrialStartSample))';
        ...
        if ~isempty( obj.( fprop ) )
            switch ischar( fields.( fcell{fidx} ) )
                ...
                case 0 % Paramter value is a number
                    if length( obj.( fprop ) ) == length( obj.TrialStartSample )
                ...
                case 1 % Paramter value is a string, so we must be careful about
                    ...
                    if  iscellstr( obj.( fprop ) )   && ...
                            length ( obj.( fprop ) ) == length ( obj.TrialStartSample )
