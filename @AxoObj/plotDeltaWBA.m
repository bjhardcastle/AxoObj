...
if ~isempty(timeVector) % Only start plotting if some trials and their F0 values were returned
    ...
    meanTrials = nanmean(deltaArray , 1);
    ...
    switch errorbar
        ...
        case 1 % Plot mean and errorbar
            ...
            if isempty(flies)
				...
				fly_mean_traces = nan(populationSize,size(deltaArray,2));
				for ifidx = 1:populationSize
					fly_mean_traces(ifidx,:) = nanmean( deltaArray( (TrialFlyNum == included_flies(ifidx)), : ) , 1);
				end
        ...
        case 0 % plot individual traces and their mean
            ...
            plot(taxes, timeVector, deltaArray, 'color', brightcolor, 'LineWidth', tracewidth);
            % Plot their mean:
