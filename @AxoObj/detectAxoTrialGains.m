function chanGains = detectAxoTrialGains(obj, chanIdx, panels_refresh_rate)
...
for tidx = 1:length(trialstarts)
    ...
    % Peaks in signals:
    % 5V / 96 pixel steps should be the minimum step size in the
    % signal that corresponds to stimulus position/velocity change
    [~,locs] = findpeaks(abs(diff(c)),'MinPeakProminence',0.9*5/96);
%     locs = find(abs(diff(detrend(c)))>0.6*5/96);        
%     
...
%     
    ...
    if isempty(locs) || length(locs) < 6
        ...
        % Convert c (0-5 Volts) to C (0-2Pi radians)
        C = c*2*pi/5;
        % Then unwrap phase:
        CC= unwrap(C);
