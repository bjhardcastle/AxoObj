function [trystarts, tryends] = detectAxoTrials(obj,trialsettings)
%DETECTTRIALS Core code for detecting trial starts/ends in Abf AI Ch2
%  [trystarts, tryends] = detectTrials(obj,trialsettings)
%
% DO NOT EDIT THIS FUNCTION. 
% The main purpose of this function is to be called by 'getAxoTrials'
%
...
%
%   trystarts       - [Nx1] array of trial start times, as Abf sample index
%                      Not assigned to object. Intended to be checked.         
%
%   tryends         - [Nx1] array of trial end times, as Abf sample index
%                      Not assigned to object. Intended to be checked.
%
...
% Discard any detected peaks which are too close to a previous trial to
% be real:
dblstarts = find(diff(trystarts)<5*obj.AbfRate/1000);   %  5 ms
...
if isfield(ts,'firstTrialError') && ts.firstTrialError
    if round(mean(wstim(1:10,ts.chan)*10)) ~= 0
       ...
       elseif (length(tryends) - length(trystarts)) == 1 && (tryends(1) < trystarts(1) && tryends(2) > trystarts(1) )
           ...
           % other trials
            trystarts(2:length(trystarts)+1) = trystarts;
            trystarts(1) = tryends(1) - floor(median(tryends-trystarts));
        end        
    end
end
